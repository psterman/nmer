...
/** OpenClaw Gateway 鍦板潃锛氶粯璁ゆ湰鏈虹鍙ｏ紱绔彛鍙樺寲鏃跺彲閫氳繃 ?gwPort= / ?gateway= 鎸囧畾锛屾垨鑷姩鎺㈡祴甯歌绔彛骞跺啓鍏?localStorage */
...
// 鍒濆灞曠ず鐢辨帰娴?璇诲彇 localStorage 鍚庣殑鍔ㄦ€佺綉鍏冲湴鍧€鍐冲畾
...
const i18n={
  zh:{
    ...
    viewDetails:"[鏌ョ湅璇︽儏]",
    close:"鍏抽棴",
    sysGroupHeader:"绯荤粺涓庤繛鎺?,
    signalPanels:"淇″彿闈㈡澘",
    radar:"鑳藉姏闆疯揪",
    wordCloud:"璇嶄簯",
    sessionList:"浼氳瘽鍒楄〃",
    bootTitle:"OpenClaw 鍚姩涓?,
    statusLoading:"姝ｅ湪鍔犺浇鏈湴鏁版嵁鈥?,
    statusFetch:"姝ｅ湪鎷夊彇浜戠鏁版嵁鈥?,
    statusProcess:"姝ｅ湪澶勭悊鐢诲儚涓庡崱鐗団€?,
    statusOnline:"宸插氨缁?,
    statusFailed:"鍔犺浇澶辫触锛屼娇鐢ㄧ┖鏁版嵁銆?,
    topTitle:"OpenClaw 鏁版嵁鎶ュ憡",
    topSub:"閬ユ祴涓婅: 娲昏穬 // 鏉ユ簮: OpenClaw_Nexus",
    linkReport:"浣撴鎶ュ憡",
    linkStats:"PK鍦板浘",
    hudTerminal:"鏁版嵁缁堢",
    levelMap:{S:"浼犺 OpenClaw",A:"涓撲笟 OpenClaw",B:"杩涢樁 OpenClaw",C:"鍒濈骇 OpenClaw",D:"OpenClaw 鏂版墜"},
    workRhythmMap:{"night-owl":"澶滅尗瀛?,morning:"鏃╄捣",afternoon:"涓嬪崍妗?,evening:"澶滈棿妗?,unknown:"鏈煡"},
    tokenSourceMap:{thinking:"鍩轰簬鎬濊€冧及绠?,messages:"鍩轰簬娑堟伅浼扮畻","sessions.json":"sessions.json",usage:"鍘熷鐢ㄩ噺",estimate:"浼扮畻"},
    partialDataHint:"缂哄皯 exitCode",
    dataSourceTitle:"鏁版嵁鏉ユ簮",
    dataSourceDesc:"浠ヤ笅鎸囨爣鏉ヨ嚜 openclawPortrait 涓?sessions.json 鐨勬眹鎬汇€俆oken/缂撳瓨/妯″瀷/娓犻亾/鏃堕棿/鎶€鑳?宸ュ叿/涓婁笅鏂?蹇冭烦/鐘舵€佸強 System Prompt 缁熻鍦ㄤ笂浼?sessions.json 鏃跺嚭鑷鏂囦欢銆?,
    dataSourcePayloadHint:"銆愯鏄庛€戝綋鍓嶅姞杞藉埌鐨?OpenClaw 鍒嗘瀽杞借嵎涓嶅畬鏁达紙鐢诲儚 dimensions銆乧omposite 涓庝細璇濇眹鎬?openclawSessionsSummary 澶氫负绌猴級锛屽洜姝ゅぇ閲忓崱鐗囦細鏄剧ず銆屾殏鏃犳暟鎹€嶆垨 0鈥斺€旇繖鏄己鏁版嵁鐨勮〃鐜帮紝涓嶆槸椤甸潰娓叉煋鏁呴殰銆傚鐞嗘柟寮忥細鈶?鍦ㄧ珯鍐呬富娴佺▼閫夋嫨 OpenClaw 寮曟搸瀹屾垚涓€娆″垎鏋愶紙浼氬啓鍏?localStorage 閿?vibe_openclaw_analysis_cache锛屽惈 openclawPortrait 绛夛級锛涒憽 淇濊瘉鏈満 Gateway 鑳介€氳繃 /api/openclaw/latest 绛夎繑鍥炰笂杩扮粨鏋勭殑 JSON锛坣pm run dev 浼氬璺緞鍥炴簮锛夛紱鈶?鎴栧皢鍚堣 JSON 瀛樹负 ~/.openclaw/last_analysis_data.json 渚涘紑鍙戞湇璇诲彇銆傛寜 F12 鍙湪鎺у埗鍙版煡鐪?[openclaw2] 杞借嵎璇婃柇鏃ュ織銆?,
    cardModalLabels:{source:"鏁版嵁鏉ユ簮",analysis:"鏁板€煎垎鏋?,calculation:"璁＄畻鏂规硶",ownerTrait:"涓讳汉鐗圭偣"},
    cardHelp:[
      [{source:"openclawPortrait.dimensions.toolSkillHeat.topTools 涓?taskHabit.topSkills銆乻essions.skills 姹囨€?,analysis:"灞曠ず OpenClaw 鏈€甯歌Е鍙戠殑宸ュ叿涓庢妧鑳藉叧閿瘝锛屽弽鏄犱娇鐢ㄥ亸濂戒笌鑳藉姏渚ч噸銆?,calculation:"浠?topTools/topSkills 鍙栧墠 3 椤癸紝鎸夋潈閲嶅幓閲嶅悗鎷兼帴銆?,ownerTrait:"鍙互鐪嬪嚭浣犳槸鍋忋€屽伐鍏锋祦銆嶈繕鏄€屾妧鑳芥祦銆峅penClaw 鐢ㄦ埛锛屼互鍙婂綋鍓嶉樁娈垫渶椤烘墜鐨勮兘鍔涙爣绛俱€?},{source:"portrait.composite.level 涓?composite.score",analysis:"绛夌骇鐢?composite 鍒嗘暟鏄犲皠涓?S/A/B/C/D锛屽搴旀帓琛屾娈典綅銆?,calculation:"level 鍙栬嚜鐢诲儚锛況ank 涓?L1锝濴5锛堟寜 score鈮?0/70/80/90 鍒嗘锛夈€?,ownerTrait:"浼犺 OpenClaw(S)鍒版柊鎵?D)锛屼竴鐪肩湅鍑轰綘鍦ㄦ帓琛屾鐨勬浣嶃€?},{source:"portrait.composite.score",analysis:"缁煎悎鑳藉姏鍒嗭紝鏁村悎娑堣€椼€佹ā鍨嬨€佸伐鍏枫€佷範鎯€佸仴搴风瓑缁村害銆?,calculation:"0锝?00 鐨勬暣鏁帮紝鐢辩敾鍍?composite 鐩存帴鍙栨暣銆?,ownerTrait:"鍒嗘暟瓒婇珮璇存槑鏁翠綋 OpenClaw 鑳藉姏瓒婂潎琛°€佽秺寮恒€?},{source:"portrait.dimensions 鍚勭淮搴?score锛歝onsumptionCost銆乵odelPreference銆乼oolSkillHeat",analysis:"涓夌淮鑳藉姏鍚戦噺锛屽垎鍒〃绀虹畻鍔涙秷鑰椼€佹ā鍨嬩娇鐢ㄣ€佸伐鍏疯皟鐢ㄤ笂鐨勫緱鍒嗐€?,calculation:"鍚?dimension 鐨?score 瀛楁鍙栨暣锛屾牸寮忎负 娑堣€?妯″瀷/宸ュ叿銆?,ownerTrait:"鍙湅鍑轰綘鍦ㄣ€岀儳 Token銆嶃€岀敤妯″瀷銆嶃€岀帺宸ュ叿銆嶄笂鐨勪晶閲嶇偣銆?},{source:"portrait.dimensions.taskHabit.workRhythm",analysis:"宸ヤ綔鑺傚绫诲瀷锛氬鐚瓙銆佹棭璧枫€佷笅鍗堟。銆佸闂存。绛夈€?,calculation:"habit.workRhythm 鍘熷鍊硷紝缁?workRhythmMap 缈昏瘧涓轰腑鏂囨爣绛俱€?,ownerTrait:"鍙嶆槧 OpenClaw 鐨勬椿璺冩椂娈靛亸濂斤紝鏄鐚瓙杩樻槸鏃╄捣鍨嬨€?},{source:"portrait.dimensions.taskHabit.peakHours",analysis:"娲昏穬楂樺嘲鏃舵锛堝皬鏃讹級锛屽鐢ㄤ簬涓?workRhythm 閰嶅悎鐞嗚В銆?,calculation:"habit.peakHours 鍙栧墠 3 涓?hour 鎷兼帴銆?,ownerTrait:"鐪嬪嚭浣犱竴澶╀腑绮惧姏鏈€闆嗕腑鐨勬椂闂寸偣銆?},{source:"portrait.dimensions.taskHabit.activeHours",analysis:"缁熻鎰忎箟涓婄殑娲昏穬灏忔椂鏁般€?,calculation:"habit.activeHours 鐩存帴灞曠ず銆?,ownerTrait:"鏁板€艰秺楂樿鏄庡湪鏃堕棿缁村害涓婃姇鍏ヨ秺澶氥€?},{source:"portrait.dimensions.taskHabit.score",analysis:"涔犳儻缁村害寰楀垎锛岀患鍚堣妭濂忋€佸嘲鍊笺€佹椿璺冨害绛夈€?,calculation:"taskHabit.score 鍙栨暣銆?,ownerTrait:"涔犳儻鍒嗛珮璇存槑浣跨敤鑺傚绋冲畾銆佸彲棰勬祴銆?},{source:"portrait.dimensions.stabilityHealth.score",analysis:"鍋ュ悍/绋冲畾鎬у緱鍒嗭紝涓庢垚鍔熺巼銆佸紓甯镐腑鏂巼鐩稿叧銆?,calculation:"stabilityHealth.score 鍙栨暣銆?,ownerTrait:"鍋ュ悍鍒嗛珮璇存槑浼氳瘽绋冲畾銆佸紓甯稿皯锛孫penClaw銆屼綋璐ㄣ€嶅ソ銆?}],
      [{source:"portrait.dimensions.consumptionCost.totalTokens 涓?sessions.token 姹囨€?,analysis:"鎬?Token 娑堣€楋紝浼樺厛鐢ㄧ敾鍍忥紝缂哄垯鐢?input+output 鎴?totalTokensSum銆?,calculation:"portraitTotalTokens>0 鍙栫敾鍍忥紱鍚﹀垯鍙?sessions 鐨?totalTokensSum 鎴?input+output 涔嬪拰銆?,ownerTrait:"鐪嬪嚭 OpenClaw銆屽悆浜嗗灏戙€嶇畻鍔涳紝鐢ㄩ噺澶ц鏄庝换鍔￠噸鎴栨ā鍨嬬敤寰楃寷銆?},{source:"sessions.token.inputTokensSum / outputTokensSum",analysis:"杈撳叆涓庤緭鍑?Token 鐨勫垎鍒眹鎬汇€?,calculation:"sessions.token 瀵瑰簲瀛楁鐩存帴姹傚拰銆?,ownerTrait:"杈撳叆澶氬亸銆岃闀挎枃/闀夸笂涓嬫枃銆嶏紝杈撳嚭澶氬亸銆岀敓鎴愬銆嶃€?},{source:"consumption.cacheHitRate 鎴?sessions 鐨?cacheRead/input 姣斾緥",analysis:"缂撳瓨鍛戒腑鐜囷紝瓒婇珮璇存槑閲嶅涓婁笅鏂囧埄鐢ㄨ秺濂姐€?,calculation:"鐢诲儚鏈?cacheHitRate 鍒欑敤锛涘惁鍒?cacheReadSum/inputTokensSum*100%銆?,ownerTrait:"鍛戒腑鐜囬珮璇存槑 OpenClaw 浼氥€岀渷鐫€鍚冦€嶏紝閲嶅鍐呭灏戦噸澶嶇畻銆?},{source:"portrait.consumptionCost.totalCostUSD 鎴?sessions.cost.totalCostUsd",analysis:"浼扮畻鎴栧疄闄呮秷鑰楃殑缇庡厓鎴愭湰銆?,calculation:"浼樺厛鐢诲儚 totalCostUSD锛屽惁鍒?sessions.cost銆?,ownerTrait:"鍙嶆槧浣犲湪 Cursor/API 涓婄殑鑺辫垂閲忕骇銆?},{source:"sessions.token.contextTokensMax 鎴栫敾鍍忕瓑浠峰瓧娈?,analysis:"涓婁笅鏂囩獥鍙ｄ娇鐢ㄤ笂闄愶紙Token 鏁帮級銆?,calculation:"token.contextTokensMax 鎴?sessions 瀵瑰簲瀛楁銆?,ownerTrait:"绐楀彛瓒婂ぇ锛孫penClaw 鑳姐€屼竴娆＄湅瀹屻€嶇殑鍐呭瓒婂銆?},{source:"sessions.cache.cacheReadSum",analysis:"缂撳瓨璇诲彇鎬婚噺銆?,calculation:"sessions.cache.cacheReadSum銆?,ownerTrait:"璇诲緱澶氳鏄庣紦瀛樺埄鐢ㄥ厖鍒嗐€?},{source:"sessions.cache.cacheWriteSum",analysis:"缂撳瓨鍐欏叆鎬婚噺銆?,calculation:"sessions.cache.cacheWriteSum銆?,ownerTrait:"鍐欏緱澶氳鏄庢湁澶ч噺鍙鐢ㄤ笂涓嬫枃琚紦瀛樸€?},{source:"sessions.token.totalTokensSum 鎴?input+output",analysis:"浼氳瘽渚?Token 鍚堣銆?,calculation:"totalTokensSum 鎴?inputTokensSum+outputTokensSum銆?,ownerTrait:"涓庣敾鍍忔€?Token 瀵圭収锛屽彲鍒ゆ柇鏁版嵁鏄惁涓€鑷淬€?},{source:"portrait.consumptionCost.tokenSource / usedEstimate",analysis:"Token 鏁版嵁鏉ユ簮锛氬師濮嬬敤閲忋€佷及绠?娑堟伅/鎬濊€?銆乻essions.json銆?,calculation:"consumption.tokenSource 鎴?usedEstimate 鎺ㄥ鐨勬爣绛俱€?,ownerTrait:"浜嗚В鏁板€兼槸瀹炴祴杩樻槸浼扮畻锛屼究浜庤В璇诲彲淇″害銆?}],
      [{source:"portrait.dimensions.modelPreference.dominantModelId 涓?sessions.model",analysis:"浣跨敤鍗犳瘮鏈€楂樼殑妯″瀷 ID銆?,calculation:"dominantModelId 闈?unknown 鍒欑敤锛屽惁鍒?sessions.model 鎴?openclaw-default銆?,ownerTrait:"鐪嬪嚭 OpenClaw銆屼富绮€嶆槸鍝釜妯″瀷銆?},{source:"portrait.modelPreference.uniqueModelCount / totalModelCalls",analysis:"浣跨敤杩囩殑涓嶅悓妯″瀷鏁颁笌鎬昏皟鐢ㄦ鏁般€?,calculation:"uniqueModelCount 涓?totalModelCalls 鐩存帴灞曠ず銆?,ownerTrait:"妯″瀷绉嶇被澶氳鏄庝綘鍦ㄥ妯″瀷闂村垏鎹紱鎬昏皟鐢ㄥ璇存槑璇锋眰棰戠箒銆?},{source:"portrait.toolSkillHeat.topTools[0] 鎴?sessions.tools.toolNames",analysis:"璋冪敤鏈€澶氱殑宸ュ叿鍚嶃€?,calculation:"topTools[0].toolName 鎴?toolNames[0]銆?,ownerTrait:"鏈€椤烘墜宸ュ叿鍙嶆槧浣犵殑涓诲姏宸ヤ綔娴侊紙濡?search銆乧odebase锛夈€?},{source:"portrait.toolSkillHeat.toolCallsTotal 鎴?sessions.tools.sessionsWithTools",analysis:"娑夊強宸ュ叿璋冪敤鐨勪細璇濇暟鎴栨€昏皟鐢ㄦ鏁般€?,calculation:"toolCallsTotal 鎴?sessionsWithTools銆?,ownerTrait:"鏁板€奸珮璇存槑浣犲父渚濊禆宸ュ叿瀹屾垚浠诲姟銆?},{source:"sessions.model.modelProvider",analysis:"妯″瀷鎻愪緵鍟嗐€?,calculation:"sessions.model.modelProvider銆?,ownerTrait:"鐪嬪嚭 OpenClaw 鎺ョ殑鏄摢瀹躲€岀伯浠撱€嶃€?},{source:"sessions.model.model",analysis:"浼氳瘽涓褰曠殑妯″瀷 ID銆?,calculation:"sessions.model.model銆?,ownerTrait:"涓?dominant 瀵圭収鍙湅鏄惁涓€鑷淬€?},{source:"portrait.toolSkillHeat.toolKinds 鎴?toolNames.length",analysis:"浣跨敤杩囩殑宸ュ叿绉嶇被鏁般€?,calculation:"toolHeat.toolKinds 鎴?toolNames 闀垮害銆?,ownerTrait:"绉嶇被澶氳鏄庝綘銆屽伐鍏烽摼銆嶄赴瀵屻€?},{source:"topTools[0].count 鎴?sessions 宸ュ叿鐩稿叧璁℃暟",analysis:"鏈€甯哥敤宸ュ叿鐨勮皟鐢ㄦ鏁版垨浼氳瘽鍐呭伐鍏锋潯鐩暟銆?,calculation:"topTools[0].count 鎴?entriesCount銆?,ownerTrait:"鐪嬪嚭浣犲鍗曚竴宸ュ叿鐨勪緷璧栫▼搴︺€?}],
      [{source:"portrait.dimensions.stabilityHealth.score",analysis:"鍋ュ悍缁村害寰楀垎銆?,calculation:"stabilityHealth.score 鍙栨暣銆?,ownerTrait:"鏁翠綋浼氳瘽绋冲畾鎬т笌銆屼綋璐ㄣ€嶃€?},{source:"portrait.health.successRate 鎴?sessions.status.successRate",analysis:"浼氳瘽鎴愬姛鐜囷紙鏃犲紓甯哥粨鏉熺殑姣斾緥锛夈€?,calculation:"鏈?successBase 鐢?health.successRate锛涘惁鍒?status.successRate锛涘啀鍚﹀垯 (sessions-aborted)/sessions銆?,ownerTrait:"鎴愬姛鐜囬珮璇存槑 OpenClaw 璺戜换鍔″緢灏戝崐璺穿銆?},{source:"portrait.health.abnormalInterruptionRate 鎴?status.abnormalInterruptionRate",analysis:"寮傚父涓柇鐜囥€?,calculation:"鐢诲儚鎴?status 鐨勫紓甯哥巼 * 100銆?,ownerTrait:"鐜囦綆璇存槑杩愯绋冲畾锛屽皯鏈夋剰澶栦腑鏂€?},{source:"sessions.lastActiveAt / updatedAt 鎴?heartbeat.lastHeartbeatSentAt",analysis:"鏈€杩戜竴娆℃椿璺冩椂闂淬€?,calculation:"lastActiveAt 鎴?updatedAt 鎴?lastHeartbeatSentAt銆?,ownerTrait:"鐪嬪嚭 OpenClaw 鏈€杩戞槸鍚﹁繕鍦ㄣ€屽姩銆嶃€?},{source:"sessions.sessionCount",analysis:"浼氳瘽鎬绘暟銆?,calculation:"sessions.sessionCount 鎴?legacy.work_days銆?,ownerTrait:"浼氳瘽澶氳鏄庝娇鐢ㄩ娆￠珮銆佹牱鏈冻銆?},{source:"sessions.channel.lastChannel",analysis:"鏈€杩戜娇鐢ㄧ殑娓犻亾銆?,calculation:"channel.lastChannel 鏁扮粍鎷兼帴銆?,ownerTrait:"鐪嬪嚭浣犱粠鍝釜鍏ュ彛鐢?Cursor銆?},{source:"sessions.channel.originProvider / originSurface",analysis:"璇锋眰鏉ユ簮鎻愪緵鍟嗕笌鐣岄潰銆?,calculation:"originProvider 涓?originSurface 鎷兼帴銆?,ownerTrait:"鍖哄垎 IDE銆乄eb銆丄PI 绛夋潵婧愩€?},{source:"ClawController.rpcCall('tasks.list') 鎴?HTTP GET /api/tasks锛涚紦瀛?openclawTasksSummary.scheduled_tasks_count",analysis:"褰撳墠 Gateway 涓婂凡閰嶇疆鐨勫畾鏃?鍛ㄦ湡浠诲姟鏉℃暟锛坈ron銆乮nterval銆乻chedule 绛夛級銆?,calculation:"浼樺厛鐢ㄥ搷搴斿瓧娈?scheduled_tasks_count锛涘惁鍒欏 tasks 鍒楄〃閫愰」鍒ゅ畾 schedule/cron/interval銆?,ownerTrait:"鏉℃暟澶氳鏄庤嚜鍔ㄥ寲宸ヤ綔娴佸銆佸悗鍙颁緥琛屼换鍔￠噸銆?}],
      [{source:"sessions.channel.lastChannel",analysis:"鏈€杩戞笭閬撱€?,calculation:"channel.lastChannel 鎷兼帴銆?,ownerTrait:"鍚屽仴搴蜂富棰樻笭閬撱€?},{source:"sessions.token.contextTokensMax",analysis:"涓婁笅鏂?Token 涓婇檺銆?,calculation:"鍚屼笂銆?,ownerTrait:"鍚岀畻鍔涗富棰樸€?},{source:"sessions.injectedWorkspaceFilesCount",analysis:"娉ㄥ叆宸ヤ綔鍖虹殑鏂囦欢鏁般€?,calculation:"sessions 瀵瑰簲瀛楁銆?,ownerTrait:"鐪嬪嚭浣犵粰 AI 寮€浜嗗灏戙€屽伐浣滃尯瑙嗛噹銆嶃€?},{source:"sessions.cache.cacheReadSum / cacheWriteSum",analysis:"缂撳瓨璇?鍐欍€?,calculation:"鍚屼笂銆?,ownerTrait:"鍚岀畻鍔涗富棰樸€?},{source:"sessions.lastActiveAt / updatedAt",analysis:"鏈€杩戞椿璺冦€?,calculation:"鍚屼笂銆?,ownerTrait:"鍚屽仴搴蜂富棰樸€?},{source:"sessions.model.modelProvider",analysis:"妯″瀷鎻愪緵鍟嗐€?,calculation:"鍚屼笂銆?,ownerTrait:"鍚屾ā鍨嬩笌宸ュ叿銆?},{source:"sessions.systemPromptChars 鎴?systemPromptReport",analysis:"绯荤粺鎻愮ず璇嶅瓧绗︽暟銆?,calculation:"sessions 瀵瑰簲瀛楁銆?,ownerTrait:"绯荤粺鎻愮ず闀胯鏄庝綘瀹氬埗浜嗚緝澶氥€屼汉璁俱€嶆垨瑙勫垯銆?},{source:"tasks.list / openclawTasksSummary锛堜笌銆屽仴搴蜂笌鐘舵€併€嶅悓婧愶級",analysis:"瀹氭椂浠诲姟鏁伴噺銆?,calculation:"scheduled_tasks_count 鎴栧浠诲姟鍒楄〃瑙ｆ瀽銆?,ownerTrait:"鍚屽仴搴蜂富棰樺畾鏃朵换鍔″崱鐗囥€?},{source:"sessions.heartbeat.lastHeartbeatText",analysis:"鏈€杩戜竴娆″績璺虫枃鏈憳瑕併€?,calculation:"lastHeartbeatText 鎴柇 60 瀛椼€?,ownerTrait:"鍙瑙佸皬铏炬渶杩戝湪銆屽康鍙ㄣ€嶄粈涔堢姸鎬併€?}]
    ],
    radarLabels:["娑堣€?,"妯″瀷","宸ュ叿","涔犳儻","鍋ュ悍","缁煎悎"],
    rowLabels:["浼氳瘽鏁?,"杈撳叆Token","杈撳嚭Token","Token鎬婚噺","缂撳瓨璇?鍐?,"妯″瀷鎻愪緵鍟?,"妯″瀷","鏈€杩戞笭閬?,"鏉ユ簮","鏈€杩戞椿璺?,"涓婁笅鏂囦笂闄?,"宸ュ叿绉嶇被鏁?,"浣跨敤宸ュ叿浼氳瘽鏁?,"宸ュ叿鍚?,"蹇冭烦鏃堕棿","蹇冭烦鍐呭","瀹氭椂浠诲姟鏁伴噺","systemSent","compactionCount","绯荤粺鎻愮ず瀛楁暟","娉ㄥ叆宸ヤ綔鍖烘枃浠?,"鎶€鑳?],
    themeNames:["鏍稿績淇″彿","绠楀姏娑堣€?,"妯″瀷涓庡伐鍏?,"鍋ュ悍涓庣姸鎬?,"浼氳瘽鏍稿績","瀹炴椂鍚庡彴","鏌ョ湅璇︽儏"],
    gatewaySwitchLabel:"棰勮绔彛",
    gatewayOptDefault:"18789",
    gatewayOptQclaw:"18790",
    gatewayOptCustom:"鑷畾涔?,
    gatewayApply:"搴旂敤骞堕噸杩?,
    gatewayProbe:"鎵弿鍦ㄧ嚎绔彛锛堟帰娴嬪垪琛ㄥ唴澶氬疄渚嬶級",
    gatewayProbeShort:"鎵弿",
    gatewayApplyShort:"搴旂敤",
    gatewayPortPlaceholder:"绔彛",
    gatewayDiscoveredLabel:"妫€娴嬪埌鐨勫疄渚?,
    gatewayDiscoveredPlaceholder:"鎵弿鍚庨€夋嫨瀹炰緥鈥?,
    gatewayInlineTitle:"缃戝叧",
    themes:[
      [{t:"OpenClaw 鏈€鐖辩敤鐨勫叧閿瘝",d:"宸ュ叿涓庢妧鑳芥縺娲讳俊鍙?},{t:"绛夌骇璇勫畾",d:null},{t:"缁煎悎鑳藉姏鍊?,d:"缁煎悎寰楀垎"},{t:"鑳藉姏鍚戦噺",d:"娑堣€?妯″瀷/宸ュ叿"},{t:"宸ヤ綔鑺傚",d:null},{t:"宄板€兼椂娈?,d:null},{t:"娲昏穬鏃堕暱",d:null},{t:"涔犳儻鍒?,d:"taskHabit.score"},{t:"鍋ュ悍鍒?,d:"stabilityHealth.score"}],
      [{t:"OpenClaw Token 鎬婚噺",d:"鐢诲儚+浼氳瘽姹囨€?},{t:"杈撳叆/杈撳嚭",d:null},{t:"缂撳瓨鍛戒腑鐜?,d:null},{t:"鑺辫垂(缇庡厓)",d:"consumption.totalCostUSD"},{t:"涓婁笅鏂囩獥鍙?,d:"token.contextTokensMax"},{t:"缂撳瓨璇诲彇",d:"cacheReadSum"},{t:"缂撳瓨鍐欏叆",d:"cacheWriteSum"},{t:"Token鍚堣",d:"totalTokensSum"},{t:"鏁版嵁鏉ユ簮",d:null}],
      [{t:"OpenClaw 鏈€甯哥敤鐨勬ā鍨?,d:null},{t:"妯″瀷鍒嗗竷",d:"涓嶅悓妯″瀷/鎬昏皟鐢?},{t:"鏈€椤烘墜宸ュ叿",d:null},{t:"宸ュ叿璋冪敤",d:null},{t:"妯″瀷鎻愪緵鍟?,d:null},{t:"妯″瀷ID",d:"model id"},{t:"宸ュ叿绉嶇被鏁?,d:null},{t:"宸ュ叿璋冪敤娆℃暟",d:null}],
      [{t:"鍋ュ悍鍒?,d:"stabilityHealth.score"},{t:"鎴愬姛鐜?,d:"鍋ュ悍/鐘舵€佹垚鍔熺巼"},{t:"寮傚父涓柇鐜?,d:null},{t:"鏈€杩戞椿璺?,d:"sessions.updatedAt / heartbeat"},{t:"宸ヤ綔閲?,d:null},{t:"娓犻亾",d:null},{t:"鏉ユ簮",d:null},{t:"瀹氭椂浠诲姟鏁伴噺",d:"tasks.list + HEARTBEAT.md锛屾潯鏁颁笌鎽樿"}],
      [{t:"娓犻亾",d:null},{t:"涓婁笅鏂嘥oken",d:"token.contextTokensMax"},{t:"娉ㄥ叆宸ヤ綔鍖烘枃浠?,d:null},{t:"缂撳瓨璇?鍐?,d:"sessions.cache"},{t:"鏈€杩戞椿璺?,d:"sessions.updatedAt"},{t:"妯″瀷鎻愪緵鍟?,d:null},{t:"绯荤粺鎻愮ず瀛楁暟",d:null},{t:"瀹氭椂浠诲姟",d:"scheduled_tasks_count"},{t:"蹇冭烦鎽樿",d:null}]
    ]
  },
  en:{
    noData:"No data",
    ...
    gatewayDiscoveredPlaceholder:"Scan, then pick鈥?,
...
function localPayload(){
  /** 涓?main.js VIBE_OPENCLAW_CACHE 涓€鑷达細涓绘祦绋?OpenClaw 鍒嗘瀽鍙啓姝ゅ锛屼笉鍐欏叆 openclaw_analysis_data銆?*/
...
/** 涓?Gateway tasks.list / HTTP 杩斿洖瀵归綈锛氬睍寮€ result銆乸ayload銆乨ata锛堝彲澶氫竴灞?ok 鍖呰锛?*/
...
/** 鍗曟潯浠诲姟鍦ㄥ崱鐗囦笂灞曠ず鐨勭煭鏍囩锛堜緵鍋ュ悍涓婚銆屽畾鏃朵换鍔°€嶅崱鐗囷級 */
...
/** 浠?tasks.list / HTTP 浠诲姟鍒楄〃涓瓫鍑哄畾鏃朵换鍔″苟鍙栧睍绀烘枃妗?*/
...
/** API 鎶?0 浣嗗垪琛ㄩ噷浠嶈兘绛涘嚭瀹氭椂浠诲姟鏃讹紝浠ュ垪琛ㄤ负鍑?*/
...
/** 浠庤浇鑽锋垨 localStorage 缂撳瓨璇诲彇涓婃缁熻鐨勫畾鏃朵换鍔℃潯鏁?*/
...
/** HTTP GET Gateway /tasks銆?api/tasks锛堜笌 openclaw-monitor 涓€鑷达級锛涜法鍩熸椂浠呯敤 query token锛岄伩鍏嶈Е鍙?CORS 棰勬澶辫触 */
...
/**
 * 鎸?OpenClaw 鏂囨。锛欻EARTBEAT.md 涓┖鏂囦欢鎴栦粎 # 娉ㄩ噴鍒欒烦杩囧績璺筹紱闈炴敞閲婅瑙嗕负涓€鏉°€屽懆鏈熸鏌ヤ换鍔°€嶃€? * 鍙傝€冿細https://open-claw.bot/docs/cli/reference/templates/heartbeat/
 */
...
/** 娣卞害鎵弿 RPC 杩斿洖涓枒浼?HEARTBEAT 姝ｆ枃鐨勯暱瀛楃涓诧紙鍏煎鏈煡瀛楁鍚嶏級 */
...
/** 浠庡凡涓婁紶鐨?sessions 鎽樿閲岃В鏋愬績璺虫枃鏈紙缃戝叧鏈毚闇茶鏂囦欢鏃朵粛鍙湁鎽樿锛?*/
...
/** 閫氳繃 Gateway 鎷夊彇宸ヤ綔鍖?HEARTBEAT.md锛堝 RPC/HTTP 璺緞鍏煎锛?*/
...
async function workerPayload(){
  ...
  /** 浠呰姹傘€屽綋鍓嶉〉闈㈡簮銆嶄笅鐨勪笂杩拌矾寰勶紙濡?localhost:3000 鐢?Vite 妗ユ帴鍒版湰鏈?Gateway锛夈€傞粯璁?Cloudflare Worker 鏍瑰湴鍧€鏈疄鐜?/api/openclaw/*锛岃疆璇細浜х敓 404锛屾晠涓嶅啀鑷姩璇锋眰 getApiEndpoint()銆?*/
  const origins=[];
...
const hbBlock=hbLinesEffective.length?hbLinesEffective.slice(0,8).map(s=>String(s).slice(0,80)).join(' 路 '):'';
const gwBlock=gwLbl.length?gwLbl.slice(0,10).map(s=>String(s).slice(0,80)).join(' 路 '):'';
let scheduledTasksNum;
/* 宸茶繛 Gateway 涓旀眹鎬诲彲淇℃椂浼樺厛 tasks.list锛涙湭缃俊鐨?0锛堣В鏋愮┖/澶辫触锛変笉鍘嬪埗 HEARTBEAT 涓庣紦瀛?*/
...
const no=(typeof L!=="undefined"&&i18n[L])?i18n[L].noData:"鏆傛棤鏁版嵁";
...
function getCardDesc(i,j,m,L){const g=i18n[L];if(!g||!g.themes[i]||!g.themes[i][j])return"";const d=g.themes[i][j].d;if(d)return d;if(i===0&&j===4)return L==="zh"?`楂樺嘲 ${m.peak} 路 娲昏穬 ${m.active}`:`peak ${m.peak} 路 active ${m.active}`;if(i===1&&j===1)return L==="zh"?`鍚堣 ${m.sumTok}`:`total ${m.sumTok}`;if(i===1&&j===2)return L==="zh"?`璇?${m.cacheRead} / 鍐?${m.cacheWrite}`:`read ${m.cacheRead} / write ${m.cacheWrite}`;if(i===2&&j===0)return `share ${m.modelShare}`;if(i===2&&j===1)return L==="zh"?"涓嶅悓妯″瀷/鎬昏皟鐢?:`unique / total calls`;if(i===2&&j===2)return `count ${m.topToolCount}`;if(i===2&&j===3)return L==="zh"?`宸ュ叿绉嶇被 ${m.toolKinds}`:`tool kinds ${m.toolKinds}`;if(i===2&&j===4)return `raw model: ${m.modelRaw}`;if(i===3&&j===4)return `channel ${m.channel}`;if(i===4&&j===0)return m.origin;if(i===4&&j===2)return L==="zh"?`绯荤粺鎻愮ず瀛楁暟 ${m.spChars}`:`system prompt chars ${m.spChars}`;if(i===4&&j===3)return "sessions.cache";if(i===4&&j===4)return "sessions.updatedAt";return "";}
...
const c3=[{t:themesArr[3][0].t,v:fmt(m.health),d:themesArr[3][0].d||"stabilityHealth.score"},{t:themesArr[3][1].t,v:m.success,d:(themesArr[3][1].d||"health.successRate / status.successRate")+(m.partialDataHint?" 路 "+m.partialDataHint:"")},{t:themesArr[3][2].t,v:m.abnormal,d:themesArr[3][2].d||"health.abnormalInterruptionRate"},{t:themesArr[3][3].t,v:m.last,d:themesArr[3][3].d||"sessions.updatedAt / heartbeat"},{t:themesArr[3][4].t,v:`${m.sessCnt} ${L==="zh"?"娆′細璇?:"sessions"}`,d:getCardDesc(3,4,m,L)},{t:themesArr[3][5].t,v:m.channel,d:themesArr[3][5].d||""},{t:themesArr[3][6].t,v:m.origin,d:themesArr[3][6].d||""},{t:themesArr[3][7].t,v:m.healthScheduledTasksCard||m.scheduledTasksCount,d:themesArr[3][7].d||""}];
const c4=[{t:themesArr[4][0].t,v:m.channel,d:getCardDesc(4,0,m,L)},{t:themesArr[4][1].t,v:m.ctx,d:themesArr[4][1].d||"token.contextTokensMax"},{t:themesArr[4][2].t,v:m.injected,d:getCardDesc(4,2,m,L)},{t:themesArr[4][3].t,v:`${m.cacheRead} / ${m.cacheWrite}`,d:getCardDesc(4,3,m,L)},{t:themesArr[4][4].t,v:m.last,d:getCardDesc(4,4,m,L)},{t:themesArr[4][5].t,v:m.provider,d:themesArr[4][5].d||""},{t:themesArr[4][6].t,v:m.spChars,d:themesArr[4][6].d||""},{t:themesArr[4][7].t,v:m.scheduledTasksCount,d:themesArr[4][7].d||""},{t:themesArr[4][8].t,v:(m.heartbeatText||"--").slice(0,50)+(m.heartbeatText&&m.heartbeatText.length>50?"鈥?:""),d:themesArr[4][8].d||""}];
...
/** 鍘绘帀缃戝叧甯歌鐨?{ ok, data } / { success, data } 澶栧眰锛屼究浜庣粺涓€瑙ｆ瀽 */
...
/* ===== LiveController: 瀹炴椂鍚庡彴鏍囩鏁版嵁杞涓庢覆鏌?===== */
const LiveController={
  ...
  start(){
    this.stop();
    this._setSessionsHint(this._lang()==='zh'?'姝ｅ湪鍔犺浇浼氳瘽鈥?:'Loading sessions鈥?);
  ...
  /* 鈹€鈹€ 涓€娆℃€ф媺鍙栨墍鏈夋憳瑕侊紙鍚?models 渚涚渷閽辩増鍧楋級 鈹€鈹€ */
  async _pollAll(){
    const $=id=>document.getElementById(id);
    /* 鏃犺鏄惁宸茶璇侀兘鍏堟洿鏂般€屾柟渚裤€嶇増鍧楄繛鎺ョ姸鎬?*/
    ...
    try{
      ...
      if(sessPack&&sessPack.data)sessionJustAutoSelected=!!this._renderSessionList(sessPack.data);
        ...
        let msg=L==='zh'?'宸茶繛鎺ワ紝浣嗙綉鍏虫湭杩斿洖鍙垪涓剧殑浼氳瘽鍒楄〃銆傚凡鏄剧ず榛樿浼氳瘽 main锛堜笌鍙充晶鑱婂ぉ鍙戦€侀粯璁や竴鑷达級锛屽彲鐐瑰嚮灏濊瘯鍔犺浇鍘嗗彶銆?:'Connected, but no session list from the gateway. Showing default session main (same as chat send).';
        ...
        else if(sessPack&&sessPack.source==='none')msg+=L==='zh'?' 甯歌鍘熷洜锛氱綉鍏虫湭瀹炵幇 sessions 鍒椾妇 RPC/REST锛屾垨杩斿洖 JSON 褰㈡€佷笌瑙ｆ瀽鍣ㄤ笉鍏煎銆?:' Common causes: no sessions list API, or an unsupported JSON shape.';
        this._setSessionsHint(msg);
      }
      /* 鍚屼竴浼氳瘽宸查€夋椂鐢辨澶勬媺鍙栨渶鏂?chat.history锛坃selectSession 宸蹭細鍔犺浇锛岄伩鍏嶉噸澶嶏級 */
      ...
      this._setSessionsHint((this._lang()==='zh'?'浼氳瘽鍔犺浇澶辫触锛?:'Failed to load sessions: ')+(e&&e.message?e.message:String(e)));
    }
  },
  ...
  _updateStatusBar(statusRes,healthRes,sessRes,tasksRes,usageRes,presRes){
    ...
    if(healthEl){
      ...
      if(healthRes&&healthRes.status==='fulfilled'&&healthRes.value){
        ...
        healthTxt=ok?(L==='zh'?'姝ｅ父':'OK'):(L==='zh'?'寮傚父':'Error');
  ...
  /* 鈹€鈹€ 鏇存柊鐢ㄩ噺鏄庣粏锛圱oken 鎬婚噺 / 杈撳叆 / 杈撳嚭锛?鈹€鈹€ */
  ...
  /* 鈹€鈹€ 鏇存柊鏂版墜涓夌増鍧楋細鐪侀挶 / 鏂逛究 / 瀹炵敤 鈹€鈹€ */
  _updateQuickBlocks(statusRes,usageRes,modelsRes){
    ...
    if(connEl){
      ...
      if(ClawController){
        if(ClawController.authenticated){connTxt=L==='zh'?'宸茶繛鎺?:'Linked';connClass='linked';}
        else if(ClawController.ws&&ClawController.ws.readyState===WebSocket.CONNECTING){connTxt=L==='zh'?'杩炴帴涓?:'Connecting';connClass='connecting';}
        else{connTxt=L==='zh'?'鏈繛鎺?:'Offline';connClass='offline';}
      }
    ...
    if(modelsEl){
      if(modelsRes&&modelsRes.status==='fulfilled'&&modelsRes.value){
        ...
        if(list.length)modelsEl.textContent=list.slice(0,12).map(m=>typeof m==='string'?m:(m.id||m.name||m.model||'')).filter(Boolean).join(', ')+(list.length>12?' 鈥?:'');
        else modelsEl.textContent=L==='zh'?'鏆傛棤':'None';
  ...
  _renderSessionList(data){
    ...
    if(!sessions.length){
      container.innerHTML='';
      if(emptyEl){container.appendChild(emptyEl);emptyEl.style.display='block';emptyEl.textContent=this._lang()==='zh'?'鏆傛棤浼氳瘽鏁版嵁':'No sessions';}
      return false;
    }
    ...
    if(loadSelect){
      const L=this._lang();
      if(loadLabel)loadLabel.textContent=L==='zh'?'鍔犺浇浼氳瘽':'Load session';
      loadSelect.innerHTML='<option value="">'+(L==='zh'?'閫夋嫨浼氳瘽鈥?:'Select session鈥?)+'</option>';
      sessions.forEach(s=>{
        const key=s.sessionKey||s.key||s.id||'unknown';
        const opt=document.createElement('option');
        opt.value=key;
        opt.textContent=key.length>36?key.slice(0,36)+'鈥?:key;
      ...
      if(toSelect){
        ...
        /* 浠呴娆℃垨褰撳墠閫変腑宸蹭笉鍦ㄥ垪琛ㄦ椂鍒囨崲閫変腑锛涘悓涓€浼氳瘽鐢?_pollAll 鏈熬 _loadSessionDetail 鍒锋柊 */
    ...
    if(sessions.length){
      /* 鏃?chat-load-session 鏃朵粛瑕侀娆℃墦寮€璇︽儏 */
      const lastKey=typeof localStorage!=='undefined'?localStorage.getItem('openclaw2_lastSessionKey'):null;
      const toSelect=lastKey&&sessions.some(s=>(s.sessionKey||s.key||s.id)===''+lastKey)?lastKey:(sessions[0]&&(sessions[0].sessionKey||sessions[0].key||sessions[0].id));
      const currentKey=this.selectedKey&&sessions.some(s=>String(s.sessionKey||s.key||s.id)===String(this.selectedKey))?this.selectedKey:null;
  ...
  /* 鈹€鈹€ 鍔犺浇浼氳瘽璇︽儏锛氭瑕?+ 灏濊瘯 chat.history 鎷夊彇鏈€杩戝璇濆唴瀹逛笌妯″瀷 鈹€鈹€ */
  async _loadSessionDetail(key){
    ...
    if(!historyEl)return;
    const L=this._lang();
    historyEl.innerHTML='<div class="live-detail-loading" id="live-detail-loading">'+(L==='zh'?'鍔犺浇瀵硅瘽鈥?:'Loading chat鈥?)+'</div>';
    let messages=[];
    let historyErr=null;
    if(ClawController&&ClawController.authenticated){
      try{
    ...
    if(!session){
      historyEl.innerHTML=`<div style="color:var(--dim);font-size:11px;padding:8px">${L==='zh'?'鏈壘鍒拌浼氳瘽':'Session not found'}</div>`;
      if(messages.length){
        const list=document.createElement('div');
        list.className='live-chat-messages';
        list.setAttribute('aria-label',L==='zh'?'鏈€杩戝璇?:'Recent messages');
        messages.forEach(m=>{
          ...
          div.innerHTML=`<div class="live-msg-role">${isUser?(L==='zh'?'鐢ㄦ埛':'User'):(L==='zh'?'鍔╂墜':'Assistant')}${modelHint}</div><div class="live-msg-bubble">${escapeHtml(content)}</div>`;
    ...
    const rows=[];
    rows.push({k:L==='zh'?'妯″瀷':'Model',v:modelName});
    if(session.updatedAt||session.createdAt)rows.push({k:L==='zh'?'鏇存柊鏃堕棿':'Updated',v:new Date(session.updatedAt||session.createdAt).toLocaleString()});
    if(session.createdAt&&session.updatedAt!==session.createdAt)rows.push({k:L==='zh'?'鍒涘缓鏃堕棿':'Created',v:new Date(session.createdAt).toLocaleString()});
    ...
    if(messages.length){
      ...
      let t=L==='zh'?'鏃犲璇濊褰曟垨鎺ュ彛涓嶅彲鐢ㄣ€?:'No messages or API unavailable.';
      if(historyErr){
        const em=(historyErr&&historyErr.message)?String(historyErr.message):String(historyErr);
        t+=' '+em;
        if(/missing scope|operator\.write|operator\.read/i.test(em))t+=L==='zh'?' 锛坈onnect 宸茶姹?operator.read/write锛岃鍏堛€屸喓 閲嶈繛銆嶏紱浠嶅け璐ュ垯鎹?Token锛?:' (Reconnect; renew token if needed.)';
      }
      p.textContent=t;
  ...
  showOffline(){
    ...
    if(emptyEl){
      ...
      emptyEl.style.display='block';
      emptyEl.textContent=L==='zh'?('鏈繛鎺?Gateway锛岃鍏堣繛鎺?+(g?' '+g:'')):('Gateway not connected. Please connect to'+(g?' '+g:''));
    }
    const gwEl=document.getElementById('live-gw-status');
    if(gwEl)gwEl.textContent=L==='zh'?'鏈繛鎺?:'Offline';
    ...
    if(gwBlockEl)gwBlockEl.textContent=L==='zh'?'鏈繛鎺?:'Offline';
  ...
  updateLabels(L){
    const map={
      'live-sessions-title':{'zh':'瀹炴椂浼氳瘽','en':'Live Sessions'},
      'lbl-gw-status':{'zh':'Gateway','en':'Gateway'},
      'lbl-gw-health':{'zh':'鍋ュ悍','en':'Health'},
      'live-gw-restart-btn':{'zh':'閲嶅惎缃戝叧','en':'Restart gateway'},
      'lbl-token-total':{'zh':'Token 鎬婚噺','en':'Total tokens'},
      'lbl-token-in':{'zh':'杈撳叆','en':'Input'},
      'lbl-token-out':{'zh':'杈撳嚭','en':'Output'},
      'lbl-sess-cnt':{'zh':'浼氳瘽鏁?,'en':'Sessions'},
      'lbl-task-cnt':{'zh':'浠诲姟','en':'Tasks'},
      'lbl-usage':{'zh':'鐢ㄩ噺','en':'Usage'},
      'lbl-presence':{'zh':'鍦ㄧ嚎瀹炰緥','en':'Online'},
      'live-sessions-empty':{'zh':'绛夊緟 Gateway 杩炴帴...','en':'Waiting for Gateway...'},
      'live-select-hint':{'zh':'鈫?閫夋嫨涓€鏉′細璇濇煡鐪嬭鎯?,'en':'鈫?Select a session to view details'},
      'live-block-save-title':{'zh':'鐪侀挶','en':'Save $'},
      'live-block-easy-title':{'zh':'鏂逛究','en':'Convenience'},
      'live-block-use-title':{'zh':'瀹炵敤','en':'Practical'},
      'lbl-block-usage':{'zh':'鐢ㄩ噺','en':'Usage'},
      'lbl-block-models':{'zh':'妯″瀷','en':'Models'},
      'lbl-block-conn':{'zh':'杩炴帴','en':'Connection'},
      'lbl-block-gw':{'zh':'Gateway','en':'Gateway'},
      'live-refresh-models':{'zh':'鍒锋柊妯″瀷','en':'Refresh models'},
      'live-fetch-link':{'zh':'鑾峰彇瀵硅瘽閾炬帴','en':'Get dialogue link'},
      'live-copy-link':{'zh':'澶嶅埗閾炬帴','en':'Copy link'},
      'live-block-use-hint':{'zh':'宸︿晶涓轰細璇濆垪琛紝閫変竴鏉″彲鐪嬭鎯?,'en':'Sessions on the left; select one for details'},
      'live-open-chat':{'zh':'鎵撳紑鑱婂ぉ','en':'Open chat'}
    };
    Object.entries(map).forEach(([id,texts])=>{const el=document.getElementById(id);if(el)el.textContent=texts[L]||texts['zh'];});
  }
};

/* 缁戝畾瀹炴椂瑙嗗浘 close / 鍒锋柊 / 涓夌増鍧楁寜閽?*/
(function(){
  const $=id=>document.getElementById(id);
  ...
  if(gwRestartBtn)gwRestartBtn.addEventListener('click',async ()=>{
    ...
    /* 1) 灏濊瘯缃戝叧 HTTP 閲嶅惎锛堜笉渚濊禆 npm run dev锛涜嫢缃戝叧鎻愪緵 /api/restart 鎴?/restart 鍒欑敓鏁堬級 */
    for(const path of['/api/restart','/restart','/api/gateway-restart']){
      try{
        ...
        if(r.ok&&(body&&(body.ok===true||body.success===true))){
          ...
          alert(L?'缃戝叧閲嶅惎鎸囦护宸插彂閫侊紝璇风◢鍊欒繛鎺ユ仮澶?:'Gateway restart sent. Reconnect in a moment.');
          return;
        }
      }catch(_){}
    }
    /* 2) 鍚屾簮寮€鍙戞湇鍔℃墽琛屽懡浠よ锛堥渶 npm run dev锛?*/
    try{
      ...
      if(r.ok&&body&&body.ok){
        gwRestartBtn.classList.remove('loading');
        alert(L?'宸查€氳繃鏈湴鍛戒护鎵ц openclaw gateway restart锛岀綉鍏崇害鍑犵鍚庢仮澶?:'openclaw gateway restart executed. Gateway will be back in a few seconds.');
        return;
      }
      if(r.status!==404&&body&&!body.ok){
        gwRestartBtn.classList.remove('loading');
        alert(L?'鎵ц澶辫触: '+(body.error||''):'Failed: '+(body.error||''));
        return;
      }
    }catch(_){}
    /* 3) 缃戝叧 WebSocket RPC */
    if(ClawController&&ClawController.authenticated){
      try{
        ...
        gwRestartBtn.classList.remove('loading');
        alert(L?'閲嶅惎鎸囦护宸插彂閫?:'Restart command sent');
        return;
      }catch(e){
        try{
    ...
    gwRestartBtn.classList.remove('loading');
    const msg=L?'鏃犳硶杩滅▼閲嶅惎銆傝鍦ㄦ湰鏈虹粓绔墽琛屼互涓嬪懡浠わ紙鍙偣鍑诲鍒讹級锛?:'Cannot restart remotely. Run this in your terminal (click to copy):';
    const copied=L?'宸插鍒讹紝璇峰埌缁堢绮樿创骞跺洖杞?:'Copied. Paste in terminal and press Enter.';
    const copyAndAlert=()=>{
  ...
  if(btnRefreshModels)btnRefreshModels.addEventListener('click',async ()=>{
    ...
    try{
      ...
      const L=state.lang||'zh';
      const modelsEl=$('live-models-list');
      if(modelsEl)modelsEl.textContent=list.length?list.slice(0,12).map(m=>typeof m==='string'?m:(m.id||m.name||m.model||'')).filter(Boolean).join(', ')+(list.length>12?' 鈥?:''):(L==='zh'?'鏆傛棤':'None');
    }catch(_){}
  ...
  if(btnCopyLink)btnCopyLink.addEventListener('click',()=>{
    ...
    navigator.clipboard&&navigator.clipboard.writeText(url).then(()=>{
      btnCopyLink.textContent=state.lang==='zh'?'宸插鍒?:'Copied';
      setTimeout(()=>{btnCopyLink.textContent=state.lang==='zh'?'澶嶅埗閾炬帴':'Copy link';},2000);
    }).catch(()=>{});
  });
  /* 瀹炵敤锛氭墦寮€鑱婂ぉ锛堟柊鏍囩锛?*/
...
/* 鐬冲瓟 #pupil-l / #pupil-r 闂儊 r 11->15->11锛屽彂閫?鍥炶溅鏃惰Е鍙?*/
...
const ClawController={
  ...
  /* 鏈璇佹秷鎭檺娴侊細鏈€澶氭樉绀?鏉★紝涔嬪悗鎶樺彔 */
  unauthMsgCount:0,
  maxUnauthDisplay:3,

  /* 鈹€鈹€鈹€鈹€鈹€ 鑾峰彇 token 鈹€鈹€鈹€鈹€鈹€ */
  getSavedToken(){
  ...
  /* 鈹€鈹€鈹€鈹€鈹€ UI 杈呭姪 鈹€鈹€鈹€鈹€鈹€ */
  /* 浠?AI 姝ｆ枃涓彁鍙栥€屾牳蹇冧竴鍙ャ€嶏細棣栨鎴栧墠 200 瀛楋紝鐢ㄤ簬绮惧噯瀵硅瘽 */
  /* 娴佸紡缁撴潫鍚庡皢 AI 姘旀场鏀逛负銆屾牳蹇冩憳瑕?+ 灞曞紑鍏ㄦ枃銆嶇粨鏋?*/
  _finalizeAiMsg(bodyEl){
    ...
    btnExpand.textContent='灞曞紑鍏ㄦ枃';
    btnExpand.addEventListener('click',()=>{bodyEl.classList.toggle('expanded');btnExpand.textContent=bodyEl.classList.contains('expanded')?'鏀惰捣':'灞曞紑鍏ㄦ枃';});
    const btnCopy=document.createElement('button');
    btnCopy.type='button';
    btnCopy.textContent='澶嶅埗鍥炲';
    btnCopy.addEventListener('click',()=>{
      const fullText=bodyEl.querySelector('.msg-full').textContent||bodyEl.querySelector('.msg-core').textContent;
      navigator.clipboard&&navigator.clipboard.writeText(fullText).then(()=>{btnCopy.textContent='宸插鍒?;setTimeout(()=>btnCopy.textContent='澶嶅埗鍥炲',1500);}).catch(()=>{});
    });
  ...
  _coreSummary(text){
    ...
    if(firstLineEnd===-1)return t.length<=maxCore?{core:t,full:''}:{core:t.slice(0,maxCore)+'鈥?,full:t};
    ...
    return {core:firstLine.slice(0,maxCore)+'鈥?,full:t};
  },
  ...
  _getOrCreateSysGroup(){
    ...
    if(!group){
      ...
      const L=state.lang||'zh';
      const t=i18n[L];
      header.textContent=(t&&t.sysGroupHeader?t.sysGroupHeader:(L==='zh'?'绯荤粺涓庤繛鎺?:'System & connection'))+' (0)';
  ...
  _updateSysGroupHeader(group){
    ...
    const L=state.lang||'zh';
    const t=i18n[L];
    const label=t&&t.sysGroupHeader?t.sysGroupHeader:(L==='zh'?'绯荤粺涓庤繛鎺?:'System & connection');
  ...
  setStatus(s){
    ...
    const map={linked:['LINKED','linked'],offline:['OFFLINE','offline'],connecting:['CONNECTING鈥?,'connecting']};
    const [txt,cls]=map[s]||['OFFLINE','offline'];
    el.textContent=txt;
    el.className=cls;
  },
  clearHistory(){const el=$("chat-history-content");if(el){el.innerHTML='';this.unauthMsgCount=0;}},
  /** 浠?chat.history 鎺ュ彛鍝嶅簲涓В鏋愬嚭 messages 鏁扮粍锛堝吋瀹瑰绉嶅悗绔繑鍥炵粨鏋勶級 */
  ...
  /** 浠庡崟鏉℃秷鎭腑鎻愬彇灞曠ず鐢ㄦ枃鏈紙鍏煎 content 瀛楃涓层€乧ontent 鏁扮粍 parts銆乼ext銆乵essage 绛夛級 */
  ...
  /** 灏嗗疄鏃跺悗鍙版煇浼氳瘽鐨勫璇濆巻鍙叉槧灏勫埌鍙充晶 chat-history锛堝厛娓呯┖鍐嶆寜鏉¤拷鍔?user/ai锛?*/
    ...
    if(historyError){
      ...
      if(/missing scope|operator\.write|operator\.read/i.test(em)){
        this.appendMsg('sys',L==='zh'
          ?'璇风偣銆屸喓 閲嶈繛銆嶄互鐢ㄦ柊鐨?scopes 閲嶆柊鎻℃墜锛涜嫢浠嶆彁绀虹己 scope锛岃鎹㈠彂甯?operator.read / operator.write 鐨?Gateway Token銆?
          :'Click Reconnect to re-handshake; if scope errors persist, use a token granting operator.read and operator.write.');
      }
    }else if(sessionKey){
      this.appendMsg('sys',(L==='zh'?'宸插姞杞戒細璇濄€?+sessionKey+'銆嶇殑瀵硅瘽鍘嗗彶':'Loaded chat history for session "'+sessionKey+'"'));
    }
  ...
  /* 鈹€鈹€鈹€鈹€鈹€ 璁惧韬唤锛圗d25519 via crypto.subtle锛宭ocalStorage 鎸佷箙鍖栵級 鈹€鈹€鈹€鈹€鈹€ */
  _DEVICE_KEY:'openclaw-device-identity-v1',

  /* 鐢熸垚 UUID锛堝畨鍏?闈炲畨鍏ㄥ潎鍙級 */
  _uuid(){
  ...
  /* Uint8Array 鈫?base64url */
  _b64u(buf){
    return btoa(String.fromCharCode(...new Uint8Array(buf)))
      .replaceAll('+','-').replaceAll('/','_').replaceAll('=','');
  },
  /* base64url 鈫?Uint8Array */
  _fromb64u(s){
  ...
  /* 鑾峰彇鎴栧垱寤鸿澶囧瘑閽ュ锛圗d25519锛屾寔涔呭寲鍒?localStorage锛?*/
  async _getOrCreateDeviceIdentity(){
    ...
    try{
      ...
      /* deviceId = SHA-256(raw public key bytes) as hex锛屼笌瀹樻柟涓€鑷?*/
  ...
  /* 鏋勫缓绛惧悕杞借嵎锛坴2 鏍煎紡锛屼笌瀹樻柟瀹屽叏涓€鑷达級骞剁鍚?*/
  async _signDevice(identity,nonce,token,role,scopes){
    const subtle=crypto.subtle;
    const signedAtMs=Date.now();
    /* 瀵煎嚭 public key bytes 鈫?base64url锛堜綔涓?publicKey 瀛楁锛?*/
    const pubRaw=await subtle.exportKey('raw',identity.publicKey);
    const publicKeyB64u=this._b64u(pubRaw);
    /* 绛惧悕杞借嵎锛歷2|deviceId|clientId|clientMode|role|scopes|signedAtMs|token|nonce */
    const payload=[
      'v2',
      identity.deviceId,
  ...
  /* 鈹€鈹€鈹€鈹€鈹€ 鍙戦€?connect 璇锋眰锛堝畬鏁磋澶囩鍚嶏紝涓庡畼鏂瑰崗璁畬鍏ㄤ竴鑷达級 鈹€鈹€鈹€鈹€鈹€ */
  async sendConnectRequest(){
    ...
    /* 浣跨敤 "webchat" 瀹㈡埛绔?id锛堟棤闇€璁惧绛惧悕锛夛紝閫氳繃 gateway token 璁よ瘉 */
    const params={
      minProtocol:3,
      maxProtocol:3,
      client:{
      ...
      role:'operator',
      /* chat.history 绛夛細缃戝叧甯歌姹?operator.read + operator.write */
    ...
    console.log('[ClawCtrl] sendConnectRequest token=',savedToken?savedToken.substring(0,8)+'鈥?:'none');
    try{
      this.ws.send(JSON.stringify(connectMsg));
      this.appendMsg('sys','鍙戦€佽璇佽姹?(token: '+(savedToken?savedToken.substring(0,8)+'鈥?:'鏃?'));
    }catch(e){
      this.appendMsg('err','鍙戦€佽璇佽姹傚け璐? '+e.message);
    }finally{
      this._connectSending=false;
    }
  },

  /* 鈹€鈹€鈹€鈹€鈹€ 璁よ瘉鎻℃墜澶勭悊 鈹€鈹€鈹€鈹€鈹€ */
  handleHandshake(msg){
    const p=msg.payload||msg.params||msg.result||{};
    const ev=msg.event||msg.type||msg.method||'';

    /* 1. connect.challenge 鈫?鏀跺埌鎸戞垬锛屽洖澶?connect */
    if(ev==='connect.challenge'){
      clearTimeout(this.handshakeTimeout);
      this.challengeReceived=true;clearTimeout(this._challengeTimeout);
      const nonce=p.nonce||'';
      this._pendingNonce=nonce;  /* 淇濆瓨 nonce 渚涚鍚嶄娇鐢?*/
      this._connectSending=false; /* 閲嶇疆闃查噸鍏ユ爣蹇楋紝鍏佽鏈鍙戦€?*/
      this.appendMsg('sys','鏀跺埌鎸戞垬 (nonce: '+nonce.substring(0,8)+(nonce.length>8?'鈥?:'')+')');
      this.sendConnectRequest().catch(e=>this.appendMsg('err','璁よ瘉鍙戦€佸け璐? '+e.message));
      return;
    }

    /* 2. JSON-RPC connect 鍝嶅簲锛坕d 浠?connect- 寮€澶达級*/
    if(typeof msg.id==='string'&&msg.id.startsWith('connect-')){
      ...
      if(msg.error||msg.ok===false){
        ...
        if(isPairing){
          this.appendMsg('sys','璁惧闇€閰嶅瀹℃壒锛佽鍦?'+OpenClawGateway.httpBase()+' 鍚庡彴鎵瑰噯锛岀劧鍚庣偣鍑汇€屸喓 閲嶈繛銆嶃€?);
        }else{
          this.appendMsg('err','璁よ瘉澶辫触: '+errMsg);
          this.appendMsg('sys','璇风‘璁?Token 姝ｇ‘锛屾垨鐣欑┖灏濊瘯鏈湴妯″紡锛堝鏈嶅姟鍣ㄥ凡閰嶇疆 trusted-proxy锛夈€?);
        }
  ...
  _onAuthSuccess(result){
    ...
    this.appendMsg('sys','鉁?璁よ瘉鎴愬姛'+(methods?' (methods: '+methods+', events: '+events+')':''));
  ...
  /* 鈹€鈹€鈹€鈹€鈹€ 澶勭悊鏀跺埌鐨勬秷鎭?鈹€鈹€鈹€鈹€鈹€ */
  handleMessage(raw){
    let msg;
    try{msg=JSON.parse(raw);}catch(e){this.appendMsg('err','娑堟伅瑙ｆ瀽澶辫触: '+e.message);return;}
    ...
    if(!this.authenticated){
      /* 杩囨护蹇冭烦/tick锛屽彧缁熻璁℃暟锛涘叾浠栨湭璁よ瘉娑堟伅闄愰噺鏄剧ず */
      const silent=['health','tick','ping','pong'];
      if(silent.includes(ev))return;
      this.unauthMsgCount++;
      if(this.unauthMsgCount<=this.maxUnauthDisplay){
        const shortMsg=raw.length>180?raw.substring(0,180)+'鈥?:raw;
        this.appendMsg('sys','[鏈璇乚 '+shortMsg);
      }else if(this.unauthMsgCount===this.maxUnauthDisplay+1){
        this.appendMsg('sys','[鏈璇佹秷鎭繃澶氾紝宸查潤榛樸€傝妫€鏌?Token 鍚庨噸杩瀅');
      }
      return;
    }

    /* 宸茶璇佸悗鐨勬秷鎭鐞?*/
    /* 浜嬩欢鎺ㄩ€侊細event 瀛楁澶勭悊 */
    ...
    /* 鍏朵粬浜嬩欢锛氳嫢鍚?text/content 鍒欐寜 AI 鍥炲灞曠ず锛屽惁鍒欓潪 ping/pong/tick/health 鎵嶈鍏?sys */
  ...
  /* 鈹€鈹€鈹€鈹€鈹€ 杩炴帴 鈹€鈹€鈹€鈹€鈹€ */
  connect(tokenOverride){
    if(this.ws&&(this.ws.readyState===WebSocket.OPEN||this.ws.readyState===WebSocket.CONNECTING))return;
    if(this.retryCount>=this.maxRetry){
      this.appendMsg('err','宸茶揪鏈€澶ч噸璇曟鏁?'+this.maxRetry+'娆?锛岃鎵嬪姩鐐瑰嚮銆屸喓 閲嶈繛銆嶆垨妫€鏌?Token銆?);
    ...
    /* 濮嬬粓鐩磋繛 openclaw 鏈嶅姟锛圵ebSocket 鏃犲悓婧愰檺鍒讹級
       token 鍚屾椂鏀惧湪 URL query 鍜?connect 娑堟伅鐨?auth.token 涓紝纭繚鍙岄€氶亾璁よ瘉 */
    ...
    this.appendMsg('sys',(savedTok?'杩炴帴涓?(token: '+savedTok.substring(0,10)+'鈥?':'杩炴帴涓紙鏃爐oken锛?)+' [绗?+(this.retryCount+1)+'娆');
    ...
    try{
      ...
      this.handshakeTimeout=setTimeout(()=>{
        if(!this.authenticated&&this.ws){
          this.appendMsg('err','鎻℃墜瓒呮椂锛?2s锛夛紝鏈嶅姟鍙兘鏈繍琛屾垨 Token 閿欒銆?);
          this.ws.close();
        }
      },12000);

      this.ws.onopen=()=>{
        this.appendMsg('sys','WebSocket 宸茶繛鎺?);
        this._pendingNonce='';
        this._connectSending=false;
        /* 绛夊緟 challenge锛?00ms 瓒呮椂锛夛紝鍏煎鏍囧噯妯″紡鍜?trusted-proxy 妯″紡 */
        this._challengeTimeout=setTimeout(()=>{
          if(!this.challengeReceived&&this.ws&&this.ws.readyState===WebSocket.OPEN){
            this.appendMsg('sys','鏈敹鍒版寫鎴橈紝灏濊瘯 trusted-proxy 妯″紡...');
            this.sendConnectRequest().catch(e=>this.appendMsg('err','璁よ瘉澶辫触锛?+e.message));
          }
        },500);
      };
      ...
      this.ws.onerror=(e)=>{
        ...
        this.setStatus('offline');
        this.appendMsg('err','WebSocket 杩炴帴閿欒锛岃纭 OpenClaw 鏈嶅姟杩愯浜?'+OpenClawGateway.getDisplay());
      };
      ...
      this.ws.onclose=(e)=>{
        ...
        this.appendMsg('sys','鏂紑 (code: '+e.code+(reason?' 路 '+reason.substring(0,60):'')+')');
        if(isAuthFail){
          this.retryCount=this.maxRetry;
          this.showTokenPanel(true);
          this.appendMsg('sys','Token 璁よ瘉澶辫触銆傝閲嶆柊杈撳叆姝ｇ‘鐨?Gateway Token銆?);
        }else if(this.retryCount<this.maxRetry){
          this.retryCount++;
          const delay=Math.min(2000*this.retryCount,10000);
          this.appendMsg('sys','灏嗗湪 '+(delay/1000).toFixed(0)+'s 鍚庤嚜鍔ㄩ噸杩炩€?);
      ...
      this.setStatus('offline');
      this.appendMsg('err','鏃犳硶寤虹珛 WebSocket: '+e.message);
      this.retryCount++;
    }
  },

  /* 鈹€鈹€鈹€鈹€鈹€ 鎵嬪姩閲嶈繛 鈹€鈹€鈹€鈹€鈹€ */
  reconnect(){
    clearTimeout(this.retryTimer);
    this.retryCount=0;
  ...
  /* 鈹€鈹€鈹€鈹€鈹€ Token 闈㈡澘寮€鍏?鈹€鈹€鈹€鈹€鈹€ */
  showTokenPanel(show){
    const panel=$("token-panel");
    if(panel)panel.classList.toggle('show',show);
  },

  /* 鈹€鈹€鈹€鈹€鈹€ 鍙戦€佽亰澶╂秷鎭?鈹€鈹€鈹€鈹€鈹€ */
  sendMessage(text){
    const t=(text||'').trim();if(!t)return;
    if(!this.ws||this.ws.readyState!==WebSocket.OPEN){
      this.appendMsg('err','鏈繛鎺ワ紝姝ｅ湪灏濊瘯閲嶈繛鈥?);
      this.reconnect();return;
    }
    if(!this.authenticated){
      this.appendMsg('err','灏氭湭璁よ瘉锛岃绛夊緟杩炴帴鎴栭噸鏂板～鍐?Token銆?);return;
    }
    blinkEyes();
    ...
    try{
      ...
      /* OpenClaw Gateway chat.send API 鏍煎紡 */
      const msg={
        type:'req',
        id:msgId,
        method:'chat.send',
      ...
      this.appendMsg('err','鍙戦€佸け璐? '+e.message);
    }
  },

  /* 鈹€鈹€鈹€鈹€鈹€ HTTP API 璋冪敤锛堝閫夛紝鐩存帴璋冪敤鍚庡彴 REST 绔偣锛?鈹€鈹€鈹€鈹€鈹€ */
  ...
  /* 鈹€鈹€鈹€鈹€鈹€ 閫氳繃 WebSocket JSON-RPC 璋冪敤鍚庡彴鏂规硶 鈹€鈹€鈹€鈹€鈹€ */
  rpcCall(method, params={}){
    return new Promise((resolve,reject)=>{
      if(!this.ws||this.ws.readyState!==WebSocket.OPEN||!this.authenticated){
        reject(new Error('鏈繛鎺?));return;
      }
...
function bind(){
  ...
  /* 鈹€鈹€ 浠?URL hash 鎴?query 璇诲彇骞朵繚瀛?Token锛堜笌瀹樻柟 Dashboard 鍏煎锛?鈹€鈹€ */
  (function(){
  ...
  /* 鈹€鈹€ 瀵硅瘽閾炬帴锛氶€氳繃 /api/dialogue-token 浠庢湰鍦版枃浠舵垨 openclaw dashboard --no-open 鑾峰彇 token 骞剁粍鍚?URL 鈹€鈹€ */
    ...
    if(url){
      ...
      if(btnCopy){
        btnCopy.onclick=function(){
          navigator.clipboard&&navigator.clipboard.writeText(url).then(()=>{
            btnCopy.textContent='宸插鍒?;btnCopy.classList.add('copied');
            setTimeout(()=>{btnCopy.textContent='澶嶅埗閾炬帴';btnCopy.classList.remove('copied');},2000);
          }).catch(()=>{});
        };
      }
  ...
  async function fetchDialogueLink(){
    ...
    setFetchStatus('姝ｅ湪浠庢湰鍦版枃浠舵垨 openclaw 鍛戒护鑾峰彇 token鈥?,false);
    try{
      ...
      if(body&&body.ok&&body.dialogueUrl){
        ...
        setFetchStatus('宸茶幏鍙栵紙鏉ユ簮: '+(body.source==='file'?'鏈湴鏂囦欢':'dashboard 鍛戒护')+'锛?,false);
      }else{
        setFetchStatus(body&&body.error?body.error:'鏃犳硶鑾峰彇瀵硅瘽閾炬帴',true);
        showDialogueLinkBox('');
      }
    }catch(e){
      setFetchStatus('璇锋眰澶辫触: '+(e&&e.message)+'銆傝纭繚鍦ㄦ湰鏈鸿繍琛屽紑鍙戞湇鍔″櫒 (npm run dev) 浠ヤ娇鐢ㄦ鍔熻兘銆?,true);
      showDialogueLinkBox('');
    }finally{
  ...
  /* 鈹€鈹€ 鑷姩鑾峰彇骞跺～鍏?Token锛堝弬鑰?token_optimization.js锛夛細鍒嗘瀽瀹岀粨鏋滃悗鎵撳紑鏈〉鏃犻渶鎵嬪姩濉啓 鈹€鈹€ */
  async function autoFetchToken(){
    ...
    setAutoStatus('姝ｅ湪鑷姩鑾峰彇 Token鈥?,false);
    try{
      /* 鏂规硶 1锛氬紑鍙戞湇鍔″櫒 /api/dialogue-token锛堣鏈湴鏂囦欢鎴?openclaw dashboard锛?*/
      try{
        const r0=await fetch(window.location.origin+'/api/dialogue-token',{cache:'no-store'});
        ...
        if(body0&&body0.ok&&body0.token){
          ...
          setAutoStatus('鉁?Token 宸茶幏鍙栵紙'+(body0.source==='file'?'鏈湴鏂囦欢':'dashboard')+'锛?,false);
          const inp=$("token-input");if(inp)inp.value=body0.token.substring(0,10)+'鈥?;
          if(body0.dialogueUrl)showDialogueLinkBox(body0.dialogueUrl);
          return body0.token;
        }
      }catch(_){}
      /* 鏂规硶 2锛氱粡鍚屾簮浠ｇ悊璁块棶 Gateway锛堣嫢缃戝叧鏆撮湶 /api/dialogue-token锛?*/
      try{
        ...
        if(r.ok){
          ...
          if(body&&body.ok&&body.token){
            ...
            setAutoStatus('鉁?Token 宸茶幏鍙栵紙Gateway锛?,false);
            const inp=$("token-input");if(inp)inp.value=body.token.substring(0,10)+'鈥?;
            return body.token;
          }
        }
      }catch(_){}
      throw new Error('鏈幏鍙栧埌 Token');
    }catch(e){
      setAutoStatus('鉁?鑷姩鑾峰彇澶辫触: '+(e&&e.message?e.message:String(e)),true);
      return null;
    }
  }
  ...
  if(btnTokenSave)btnTokenSave.addEventListener('click',()=>{
    ...
    /* 鍏堜繚瀛?token锛屽啀 reconnect锛堝彧璋冪敤涓€娆★紝閬垮厤鍙岄噸杩炴帴锛?*/
    if(tok){
      try{localStorage.setItem('openclaw_gatewayToken',tok);}catch{}
      ClawController.appendMsg('sys','Token 宸蹭繚瀛橈紝姝ｅ湪閲嶆柊杩炴帴鈥?);
    }else{
      try{localStorage.removeItem('openclaw_gatewayToken');}catch{}
      ClawController.appendMsg('sys','浠ユ湰鍦版ā寮忛噸鏂拌繛鎺モ€?);
    }
    if(tok&&typeof showDialogueLinkBox==='function')showDialogueLinkBox(buildDialogueUrlFromSaved());
    const panel=$("token-panel");
    if(panel)panel.classList.remove('show');
    ClawController.reconnect(); /* 鍙皟鐢ㄤ竴娆★紝reconnect 鍐呴儴浼氳 localStorage 涓殑 token */
  });

  /* 鈹€鈹€ agent 鏍囩鏍忥細娓叉煋鍙垏鎹㈢殑浼氳瘽鏍囩锛岀偣鍑诲嵆鍔犺浇璇ヤ細璇濊亰澶╄褰?鈹€鈹€ */
  window.renderChatAgentTabs=function(sessions,activeKey){
    ...
    sessions.forEach(s=>{
      ...
      tab.dataset.key=key;
      tab.textContent=key.length>24?key.slice(0,24)+'鈥?:key;
      tab.title=key;
  ...
  window.refreshChatLoadSession=async function(){
    ...
    const L=state.lang||'zh';
    if(lab)lab.textContent=L==='zh'?'鍔犺浇浼氳瘽':'Load session';
    if(!sel)return;
    try{
      ...
      sessions=sessions.slice().sort((a,b)=>(new Date(b.updatedAt||b.createdAt||0)).getTime()-(new Date(a.updatedAt||a.createdAt||0)).getTime());
      sel.innerHTML='<option value="">'+(L==='zh'?'閫夋嫨浼氳瘽鈥?:'Select session鈥?)+'</option>';
      sessions.forEach(s=>{
        const key=s.sessionKey||s.key||s.id||'unknown';
        const opt=document.createElement('option');
        opt.value=key;
  ...
  /* 鈹€鈹€ 鎵嬪姩閲嶈繛锛堝厛鑷姩鎺㈡祴绔彛锛岄€傞厤缃戝叧闅忔満绔彛锛?鈹€鈹€ */
  ...
  /* API 鎸夐挳鏄犲皠鍒?RPC 鏂规硶锛堝€熼壌 Control Center 鎬濊矾锛氭€昏/浼氳瘽/浠诲姟/瀹℃壒/鍛樺伐/閫氶亾锛岃嚜鏈夊疄鐜帮級 */
  const apiActionMap={
  ...
  async function runApiAction(action){
    ...
    try{
      ...
      showApiResult('閿欒: '+e.message,false);
    }finally{
      if(btn)btn.classList.remove('loading');
    }
  }
  ...
  /* 绠＄悊鍛戒护鍖烘枃妗堥殢璇█鏇存柊锛堜緵 applyLang 璋冪敤锛?*/
  function updateMgmtLabels(){
    const L=state.lang;
    const t=L==='zh'?{title:'绠＄悊鍛戒护',tasks:'浠诲姟',projects:'椤圭洰',approvals:'瀹℃壒',presence:'鍦ㄧ嚎鑺傜偣',channels:'閫氶亾',usage:'鐢ㄩ噺'}
  ...
  (async function startConnect(){
    ...
    try{
      ...
      ClawController.connect();
      if(tok){
        ClawController.appendMsg('sys','Token 宸茶嚜鍔ㄥ～鍏咃紝姝ｅ湪杩炴帴鈥?);
