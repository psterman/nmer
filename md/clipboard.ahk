...
if (!InitClipboardFTS5DB()) {
    MsgBox("数据库初始化失败！`n请检查 sqlite3.dll 是否存在。", "错误", "IconX")
    ExitApp
}
...
if (!ClipboardFTS5DB || ClipboardFTS5DB = 0) {
    MsgBox("数据库连接失败！`n请检查 Clipboard.db 文件权限。", "错误", "IconX")
    ExitApp
}

; 注册剪贴板监听器
OnClipboardChange(OnClipboardChangeHandler, 1)
RecordClipboardEvent("初始化", "剪贴板监听器已注册，数据库已连接")
...
ProcessClipboardChange(Type) {
    ...
    try {
        ...
        if (Type = 1) {
            ...
            if (!ClipboardFTS5DB || ClipboardFTS5DB = 0) {
                ; 尝试重新初始化数据库
                RecordClipboardEvent("数据库恢复", "检测到数据库未连接，尝试重新初始化")
                if (!InitClipboardFTS5DB()) {
                    RecordClipboardEvent("数据库恢复", "重新初始化失败")
                    RecordSaveResult(false, "数据库未初始化，且重新初始化失败")
                    TrayTip("错误", "数据库未初始化，请重启程序", "Iconx 2")
                    return
                }
                RecordClipboardEvent("数据库恢复", "重新初始化成功")
            }
            ...
            if (ClipboardFTS5DB && ClipboardFTS5DB != 0) {
                ...
                if (result) {
                    ...
                    if (ClipboardFTS5DB && ClipboardFTS5DB != 0) {
                        ...
                        if (errorMsg = "") {
                            errorMsg := "数据库执行失败，但未返回错误信息"
                        }
                    }
                    RecordSaveResult(false, "保存失败: " . errorMsg)
                    ; 显示错误提示
                    TrayTip("保存失败", "剪贴板内容保存失败: " . errorMsg, "Iconx 2")
                }
                } else {
                    ; 保存失败，记录详细错误信息
                    errorMsg := "未知错误"
                ...
                ; 尝试重新初始化数据库
                if (InitClipboardFTS5DB()) {
                    RecordClipboardEvent("数据库恢复", "数据库重新初始化成功")
                    ; 重新尝试保存
                    result := SaveToClipboardFTS5(content, SourceApp, detectedType)
