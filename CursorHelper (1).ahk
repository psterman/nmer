...
global ScreenshotToolbarWV2Ctrl := 0  ; 截图助手工具栏 WebView2 控制器
global ScreenshotToolbarWV2 := 0  ; 截图助手工具栏 CoreWebView2
global ScreenshotToolbarWV2Ready := false  ; 截图助手工具栏 WebView2 是否就绪
global ScreenshotToolbarWV2PaintOk := false  ; 截图助手工具栏页面是否完成首帧可见渲染
...
; 检查可能有干扰的剪贴板工具
CheckInterferingClipboardTools() {
    ; 常见的剪贴板增强工具
...
ShowScreenshotEditor(DebugGui := 0) {
    ...
    try {
        ...
        global ScreenshotEditorCloseBtn := 0  ; 先初始化为0，稍后创建
        ...
        ScreenshotToolbarWV2Ctrl := 0
        ScreenshotToolbarWV2 := 0
        ScreenshotToolbarWV2Ready := false
        ScreenshotToolbarWV2PaintOk := false
        ScreenshotToolbarNativeFallback := false
        GuiID_ScreenshotToolbar.OnEvent("Size", ScreenshotToolbar_OnSize)
        try WebView2.create(GuiID_ScreenshotToolbar.Hwnd, ScreenshotToolbar_OnCreated, WebView2_EnsureSharedEnvBlocking())
        ...
        ScreenshotToolbar_NotifyHostMemory(true)
        ...
        SetTimer(ScreenshotToolbar_EnsureUsable, -1200)
        
        ; 激活工具栏窗口
        try {
            WinActivate("ahk_id " . GuiID_ScreenshotToolbar.Hwnd)
        } catch as e {
        }
...
ScreenshotEditorDragWindow(*) {
    ...
    try {
        if (GuiID_ScreenshotEditor && GuiID_ScreenshotEditor != 0) {
            ...
            PostMessage(0xA1, 2, 0, , "ahk_id " . GuiID_ScreenshotEditor.Hwnd)
        }
    } catch as e {
        ; 如果失败，尝试直接使用窗口句柄
        try {
            if (GuiID_ScreenshotEditor && GuiID_ScreenshotEditor.Hwnd) {
...
; 工具栏拖动窗口函数
ScreenshotToolbarDragWindow(*) {
    global GuiID_ScreenshotToolbar
    
    try {
        if (GuiID_ScreenshotToolbar && GuiID_ScreenshotToolbar != 0) {
            ; 发送拖动消息（WM_NCLBUTTONDOWN with HTCAPTION = 2）
            PostMessage(0xA1, 2, 0, , "ahk_id " . GuiID_ScreenshotToolbar.Hwnd)
        }
    } catch as e {
        ; 如果失败，尝试直接使用窗口句柄
        try {
            if (GuiID_ScreenshotToolbar && GuiID_ScreenshotToolbar.Hwnd) {
                PostMessage(0xA1, 2, 0, 0, GuiID_ScreenshotToolbar.Hwnd)
            }
        } catch {
            ; 忽略错误
        }
...
ScreenshotToolbar_OnCreated(ctrl) {
    global ScreenshotToolbarWV2Ctrl, ScreenshotToolbarWV2, ScreenshotToolbarWV2Ready, ScreenshotToolbarWV2PaintOk
    ScreenshotToolbarWV2Ctrl := ctrl
    ...
    ApplyWebView2PerformanceSettings(ScreenshotToolbarWV2)
    WebView2_RegisterHostBridge(ScreenshotToolbarWV2)
    try ScreenshotToolbarWV2Ctrl.IsVisible := true
    try ScreenshotToolbarWV2.add_WebMessageReceived(ScreenshotToolbar_OnMessage)
    try ScreenshotToolbarWV2.add_NavigationCompleted(ScreenshotToolbar_OnNavigationCompleted)
    htmlPath := A_ScriptDir "\ScreenshotToolbarWebView.html"
    try {
        if FileExist(htmlPath) {
            ScreenshotToolbarWV2.NavigateToString(FileRead(htmlPath, "UTF-8"))
        } else {
            try ApplyUnifiedWebViewAssets(ScreenshotToolbarWV2)
            ScreenshotToolbarWV2.Navigate(BuildAppLocalUrl("ScreenshotToolbarWebView.html"))
        }
    } catch as e {
        try ScreenshotToolbarWV2.NavigateToString("<!doctype html><html><body style='margin:0;background:#0a0a0a;color:#ff9d3a;font:12px Segoe UI;padding:10px'>截图工具栏加载失败: " . e.Message . "</body></html>")
    }
    ScreenshotToolbar_ApplyBounds()
}

ScreenshotToolbar_EnsureCreated(*) {
    global ScreenshotToolbarWV2Ctrl
    if !ScreenshotToolbarWV2Ctrl
        ScreenshotToolbar_EnableNativeFallback("wv2_create_timeout")
}

ScreenshotToolbar_OnNavigationCompleted(sender, args) {
    try ok := args.IsSuccess
    catch
        ok := true
    if ok
...
ScreenshotToolbar_OnSize(*) {
    ScreenshotToolbar_ApplyBounds()
}

ScreenshotToolbar_ApplyBounds() {
    global GuiID_ScreenshotToolbar, ScreenshotToolbarWV2Ctrl
    if !(IsObject(GuiID_ScreenshotToolbar) && GuiID_ScreenshotToolbar != 0)
        return
    if !ScreenshotToolbarWV2Ctrl
        return
    WinGetClientPos(, , &cw, &ch, GuiID_ScreenshotToolbar.Hwnd)
    rc := WebView2.RECT()
    rc.left := 0
    rc.top := 0
    rc.right := cw
    rc.bottom := ch
    ScreenshotToolbarWV2Ctrl.Bounds := rc
}

ScreenshotToolbar_NotifyHostMemory(shown) {
    global ScreenshotToolbarWV2
    if shown
        WebView2_NotifyShown(ScreenshotToolbarWV2)
    else
        WebView2_NotifyHidden(ScreenshotToolbarWV2)
}

ScreenshotToolbar_RefreshComposition(*) {
    global ScreenshotToolbarWV2Ctrl
    if !ScreenshotToolbarWV2Ctrl
        return
    try {
        ScreenshotToolbar_ApplyBounds()
        ScreenshotToolbarWV2Ctrl.NotifyParentWindowPositionChanged()
    }
}

ScreenshotToolbar_OnMessage(sender, args) {
    global ScreenshotToolbarWV2Ready, ScreenshotToolbarWV2PaintOk
    jsonStr := args.WebMessageAsJson
    ...
    switch action {
        case "ready":
            ScreenshotToolbarWV2Ready := true
            ScreenshotToolbar_SendState()
        case "paint_ok":
            ScreenshotToolbarWV2PaintOk := true
        case "layout":
            ScreenshotToolbar_ApplyLayout(msg.Get("width", 0), msg.Get("height", 0))
        case "dragWindow":
            ScreenshotToolbarDragWindow()
        case "invoke":
            cmd := msg.Get("cmd", "")
            ScreenshotToolbar_InvokeCommand(cmd)
    }
}
...
ScreenshotToolbar_SendState() {
    global ScreenshotToolbarWV2, ScreenshotToolbarWV2Ready
    global ScreenshotEditorToolbarVisible
    if !ScreenshotToolbarWV2 || !ScreenshotToolbarWV2Ready
        return
    try ScreenshotToolbarWV2.PostWebMessageAsJson(
        WebView_DumpJson(Map("type", "state", "toolbarVisible", ScreenshotEditorToolbarVisible))
    )
}

ScreenshotToolbar_ApplyLayout(width, height) {
    global GuiID_ScreenshotToolbar, GuiID_ScreenshotEditor
    global ScreenshotToolbarCurrentWidth, ScreenshotToolbarCurrentHeight
    ...
    w := Max(240, Min(960, w))
    h := Max(40, Min(120, h))
    ScreenshotToolbarCurrentWidth := w
    ScreenshotToolbarCurrentHeight := h
    ...
    if (IsObject(GuiID_ScreenshotEditor) && GuiID_ScreenshotEditor != 0) {
        try {
            WinGetPos(&ex, &ey, , &eh, "ahk_id " . GuiID_ScreenshotEditor.Hwnd)
            tx := ex
            ty := ey + eh + 10
        } catch {
            WinGetPos(&tx, &ty, , , "ahk_id " . GuiID_ScreenshotToolbar.Hwnd)
        }
    } else {
        WinGetPos(&tx, &ty, , , "ahk_id " . GuiID_ScreenshotToolbar.Hwnd)
    }
...
ScreenshotToolbar_EnsureUsable(*) {
    global ScreenshotToolbarWV2, ScreenshotToolbarWV2PaintOk, ScreenshotToolbarWV2Ready
    if !ScreenshotToolbarWV2 {
        ScreenshotToolbar_EnableNativeFallback("wv2_missing")
        return
    }
    ; 若首帧仍未完成，切换到极简安全版 HTML，保证按钮可见
    if (!ScreenshotToolbarWV2PaintOk) {
        try ScreenshotToolbarWV2.NavigateToString(ScreenshotToolbar_BuildSafeInlineHtml())
        SetTimer(ScreenshotToolbar_RefreshComposition, -60)
        SetTimer(ScreenshotToolbar_EnsureUsableSecondPass, -700)
    }
}

ScreenshotToolbar_EnsureUsableSecondPass(*) {
    global ScreenshotToolbarWV2PaintOk
    if !ScreenshotToolbarWV2PaintOk {
        ScreenshotToolbar_EnableNativeFallback("wv2_paint_timeout")
        TrayTip("截图工具栏", "已切换到兼容渲染模式", "Iconi 1")
    }
}

ScreenshotToolbar_BuildSafeInlineHtml() {
    return "
(
<!doctype html>
...
ScreenshotToolbar_EnableNativeFallback(reason := "") {
    global GuiID_ScreenshotToolbar, ScreenshotToolbarNativeFallback
    global ScreenshotToolbarWV2Ctrl, ScreenshotToolbarWV2, ScreenshotToolbarWV2Ready, ScreenshotToolbarWV2PaintOk
    global ScreenshotToolbarCurrentWidth, ScreenshotToolbarCurrentHeight
    if !(IsObject(GuiID_ScreenshotToolbar) && GuiID_ScreenshotToolbar != 0)
        return
    if ScreenshotToolbarNativeFallback
        return
    ScreenshotToolbarNativeFallback := true
    ScreenshotToolbarWV2Ready := false
    ScreenshotToolbarWV2PaintOk := false
    try {
        if (ScreenshotToolbarWV2Ctrl)
            ScreenshotToolbarWV2Ctrl.IsVisible := false
    }
    ...
    AddBtn(txt, cmd, isDanger := false) {
        ...
        c := GuiID_ScreenshotToolbar.Add("Text", "x" . x . " y" . y . " w" . btnW . " h" . btnH . " Center 0x200 Border c" . bd . " Background" . bg, txt)
        c.SetFont("s10 Bold c" . tc, "Segoe UI")
        c.OnEvent("Click", (*) => ScreenshotToolbar_InvokeCommand(cmd))
        x += btnW + gap
    }
    AddSep() {
        global GuiID_ScreenshotToolbar
        x += sepGap
        GuiID_ScreenshotToolbar.Add("Text", "x" . x . " y" . (y + 7) . " w1 h18 Background5a3a20 c5a3a20", "")
        x += 1 + sepGap
    }
    ...
    ScreenshotToolbarCurrentWidth := Max(320, x + 8)
    ScreenshotToolbarCurrentHeight := 52
    dragTop := GuiID_ScreenshotToolbar.Add("Text", "x0 y0 w" . ScreenshotToolbarCurrentWidth . " h8 BackgroundTrans", "")
    dragBottom := GuiID_ScreenshotToolbar.Add("Text", "x0 y" . (ScreenshotToolbarCurrentHeight - 8) . " w" . ScreenshotToolbarCurrentWidth . " h8 BackgroundTrans", "")
...
SyncScreenshotToolbarPosition() {
    ...
    try {
        ...
        ; 获取主窗口位置和尺寸
        WinGetPos(&EditorX, &EditorY, &EditorW, &EditorH, "ahk_id " . GuiID_ScreenshotEditor.Hwnd)
        ...
        ; 获取工具栏当前尺寸
        WinGetPos(, , &ToolbarW, &ToolbarH, "ahk_id " . GuiID_ScreenshotToolbar.Hwnd)
        ...
        ; 如果出错，停止定时器
        SetTimer(SyncScreenshotToolbarPosition, 0)
    }
}
    ...
    try {
        ...
        if (!ScreenshotEditorToolbarVisible) {
            ...
            if (GuiID_ScreenshotToolbar != 0) {
                ScreenshotToolbar_NotifyHostMemory(false)
...
CloseScreenshotEditor() {
    ...
    global ScreenshotToolbarWV2Ctrl, ScreenshotToolbarWV2, ScreenshotToolbarWV2Ready, ScreenshotToolbarWV2PaintOk, ScreenshotToolbarNativeFallback
...
ExecuteScreenshotOCR() {
    ...
    try {
        ...
        ; 验证位图有效性
        try {
            testWidth := Gdip_GetImageWidth(ScreenshotEditorBitmap)
            testHeight := Gdip_GetImageHeight(ScreenshotEditorBitmap)
        ...
        if (cleanedText = "") {
            try {
                cleanedText := CleanCodeOCRText(Result)
            } catch as e {
