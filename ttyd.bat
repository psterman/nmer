...
set "PID="
set "PNAME="
for /f %%P in ('powershell -NoProfile -Command "$c=Get-NetTCPConnection -State Listen -LocalPort %PORT% -ErrorAction SilentlyContinue; if($c){$c[0].OwningProcess}"') do set "PID=%%P"
...
"%TTYD_EXE%" -p %PORT% -i 127.0.0.1 -W -t rendererType=dom -t fontSize=14 -w "%USERPROFILE%" cmd.exe
