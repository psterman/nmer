...
; ===================== 辅助函数 =====================
; 注意：RepeatString函数已移至全局作用域，避免重复声明

; ===================== JSON处理 =====================
; 使用Jxon库进行JSON处理
class JSON {
    static parse(jsonStr) => Jxon_Load(jsonStr)
    static stringify(obj, indent := 0) => Jxon_Dump(obj)
}
...
class CurserApp {
    ...
    CreateMainGUI() {
        ...
        ; 按钮
        MainGUI.Add("Button", "x10 y280 w80 h30 vRefreshBtn", "刷新").OnEvent("Click", (*) => this.RefreshWorkspaces())
        MainGUI.Add("Button", "x100 y280 w80 h30 vTestBtn", "测试").OnEvent("Click", (*) => this.TestExport())
    ...
    TestExport() {
        ...
        chats := this.ParseWorkspaceChats(workspace["id"])
        ...
        success := this.ExportToMarkdown(chats, filePath)
...
Main() {
    try {
        ...
        ; 确保app被正确赋值
        if (!IsSet(app) || !app) {
            throw Error("CurserApp初始化失败")
        }
    } catch error {
        ; 在AHK v2中，异常对象的属性可能不同
        errorMsg := "程序启动失败: " Type(error) "`n"
        errorMsg .= "行号: 未知`n"
        errorMsg .= "文件: 未知"
        MsgBox(errorMsg, "错误", "Iconx")
        ExitApp()
    }
}
